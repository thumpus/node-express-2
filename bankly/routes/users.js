...
const { authUser, requireLogin, requireAdmin, requireAdminOrCurrUser } = require('../middleware/auth');
...
router.patch('/:username', authUser, requireLogin, requireAdminOrCurrUser, async function(
  req,
  res,
  next
) {
  try {
    ...
    //FIXES BUG #3: makes sure the username, password, and admin fields aren't passed into the fields to update.
