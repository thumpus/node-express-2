...
describe("GET /users/[username]", function() {
  ...
  //TESTS BUG #4: There was no test to see if the request throws an error when trying to find a nonexistant user.
  test("should return an error if trying to find a nonexistant user", async function(){
    const response = await request(app)
      .get("/users/fgdsfjgsdfhlkjghsfdlkjghweaoiuhfvifhbskdjgbflskd")
      .send({_token: tokens.u1 });
...
describe("PATCH /users/[username]", function() {
  ...
  //TESTS BUG #2: this used to work just because the u3 token in this test is coming from the manual invoking of createToken() to create the tokens,
  // rather than using the token generated from the /auth/login route, which was non functional, causing Bug 2. 
  ...
  //TESTS BUG #1: There was no test that tested a non-admin matching user editing their own account

    test("should patch data if matching user", async function() {
      const response = await request(app)
        .patch("/users/u1")
        .send({ _token: tokens.u1, first_name: "new-fn1" }); //u1 is not admin but should still be able to edit themselves
      expect(response.statusCode).toBe(200);
      expect(response.body.user).toEqual({
        username: "u1",
        first_name: "new-fn1",
  ...
  //TESTS BUG #3: this was initially passing because the user wasn't able to edit their own account, therefore it was returning a 401 error. 
  // I changed the test to check for whether or not the disallowed fields were updated, the test only passing if updating the admin, username, and/or password did not work.

  test("should disallowing patching not-allowed-fields", async function() {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new-fn1", admin: true, username: "gfdgsdhgskl" });
      expect(response.body.user).toEqual({
        username: "u1",
        first_name: "new-fn1",
        last_name: "ln1",
